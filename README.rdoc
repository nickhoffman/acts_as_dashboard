...
Add some "number" widgets to the dashboard. Eg:
  ...
  class DashboardsController < ApplicationController
    ...
    dashboard_number do |dn|
      dn.name            = :this_minutes_seconds
      dn.title           = 'Seconds in this minute'
      dn.update_interval = '5s'
      dn.data            {Time.now.sec}
    end

    dashboard_number do |dn|
      dn.name            = :load_average
      dn.title           = 'Load average'
      dn.update_interval = '5s'
